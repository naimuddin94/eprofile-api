...
const {
 asyncHandler, ApiResponse, ApiError, fileUploadOnCloudinary,
} = require('../utils');
...
const createCompany = asyncHandler(async (req, res) => {
    ...
    if (
        [name, username, location, services, targetMarket].some((field) => field?.trim() === '')
    ) {
    ...
    if (req.files && Array.isArray(req.files.coverPhoto) && req.files.coverPhoto.length > 0) {
        coverPhotoUrl = await fileUploadOnCloudinary(req?.files?.coverPhoto[0]?.buffer);
    }
    ...
    if (!company) {
        return res.json(
            new ApiResponse(500, {}, 'Something went wrong while creating company to database'),
        );
    }
